...
impl Visitor {
    fn visit_local_extend(&mut self, local: &mut Local) -> (bool, Vec<Stmt>) {
        ...
        // After visit_pat_mut, `local.pat` may be Pat::Type(verus_tmp_z: u32).
        // The type is needed for x_decls declarations like `let mut z;` which
        // lose the type annotation, breaking type inference for proof_with_ret.
        let local_ty =
            if let Pat::Type(pat_type) = &local.pat { Some((*pat_type.ty).clone()) } else { None };

        for decl in visit_pat.x_decls.iter_mut() {
            if let Stmt::Local(new_local) = decl {
                new_local.attrs.extend(local.attrs.clone());
                // Re-inject type annotation for bare ident declarations
                if let Some(ref ty) = local_ty {
                    if let Pat::Ident(_) = &new_local.pat {
                        if new_local.init.is_none() {
                            let pat_span = new_local.pat.span();
                            let inner = new_local.pat.clone();
                            new_local.pat = Pat::Type(PatType {
                                attrs: vec![],
                                pat: Box::new(inner),
                                colon_token: token::Colon { spans: [pat_span] },
                                ty: Box::new(ty.clone()),
                            });
                        }
                    }
